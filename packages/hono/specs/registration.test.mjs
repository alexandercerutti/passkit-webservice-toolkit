...
describe("registration service", async () => {
	...
	/**
	 * @type {{ close: Function; address: string | null } | undefined}
	 */
	let serverInstance;
	...
	describe("import", async () => {
		it("should import plugin from direct import", async () => {
			const { default: directFileImport } = await import(
				"hono-passkit-webservice/v1/registration.js"
			);
		...
		it("should import plugin from v1 entry point", async () => {
			const { RegistrationRouter } = await import("hono-passkit-webservice/v1");
		...
		it("should import v1 plugin from global package entry", async () => {
			...
			} = await import("hono-passkit-webservice");
